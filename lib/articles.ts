...
function validateBlock(raw: unknown, file: string, index: number): Block {
  ...
  switch (raw.type) {
    ...
    case "youtube": {
      // L'identifiant est validé strictement : c'est lui qui entre dans l'URL
      // de l'iframe, jamais une URL complète venue du JSON.
      const id = requireString(raw, "id", where);
      if (!/^[A-Za-z0-9_-]{11}$/.test(id)) {
        fail(where, `l'« id » d'un bloc youtube doit être un identifiant YouTube valide (11 caractères), reçu « ${id} »`);
      }
